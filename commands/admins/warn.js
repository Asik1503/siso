 ...
 module.exports.run = async (bot, message, args) => {
    ...
    User.findOne({userID: memb.id}, (err, res) => {
        ...
        if(res){
            ...
            res.warn = parseInt(Date.now() + ms(args[1]))
            res.save()
            bot.panel(null, `участнику ${memb.user} было выдано предупреждение на **${convertTime(parseInt(Date.now() + ms(args[1])))}** по причине: **${reas}**`, null, null, "admin", true)
        }
    })
}
