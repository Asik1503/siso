...
module.exports.run = async (bot, message, args) => {
    User.findOne({userID: message.author.id}, (err, res) => {
        ...
        if(res) {
            ...
            if(!memPerms) return bot.panel(null, `Участники и так не видят вашу комнату`, null, null, "panel", null, 15)

            ch.updateOverwrite(cfg.memberRole, {
                VIEW_CHANNEL: false
            })
            bot.panel(null, `Вы успешно скрыли комнату от других участников`, null, null, "panel")
        }
    })
}